...
st.markdown("""
이 대시보드는 **기온, 습도, 강수량** 등 주요 기상 데이터를 시각적으로 분석하기 위해 만들어졌습니다.  
월별, 지역별, 계절별 날씨 변화를 한눈에 확인할 수 있습니다.
""")
...
with st.sidebar:
    ...
    metrics = st.multiselect("지표 선택", ["기온(℃)", "습도(%)", "강수량(mm)"], default=["기온(℃)", "습도(%)"])
...
selected_metric = st.selectbox("비교할 지표 선택", ["기온(℃)", "습도(%)", "강수량(mm)"])

fig2 = px.bar(
    filtered.groupby("지역")[selected_metric].mean().reset_index(),
    x="지역", y=selected_metric,
    color="지역",
    text_auto=".2s"
)
